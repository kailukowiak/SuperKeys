...
print_chrome_copy_hint() {
    ...
    echo "bundled shell extension - see the keyd README); KDE. On COSMIC"
    echo "the mapper connects but can die on some window events (upstream"
    echo "crash in its Wayland title parsing) - check ~/.config/keyd/app.log"
    echo "if the bindings stop applying."
